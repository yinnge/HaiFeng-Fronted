...
import type { EnterpriseIndustryListVO } from '@/types/company'

const props = defineProps<{
  data: EnterpriseIndustryListVO[]
  loading: boolean
  total: number
  page: number
  size: number
...
const emit = defineEmits<{
  (e: 'detail', row: EnterpriseIndustryListVO): void
      ...
      >
        ...
        <el-table-column prop="isPrimary" label="是否主行业" width="120" align="center">
          <template #default="{ row }">
            <span :class="['status-pill', row.isPrimary ? 'status-on' : 'status-off']">
              {{ row.isPrimary ? '主行业' : '普通' }}
            </span>
          </template>
        </el-table-column>
