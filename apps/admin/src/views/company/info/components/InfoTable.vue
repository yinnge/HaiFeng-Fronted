...
const emit = defineEmits<{
  (e: 'detail', row: EnterpriseListVO): void
  (e: 'edit', row: EnterpriseListVO): void
  (e: 'toggle-status', row: EnterpriseListVO): void
  (e: 'delete', id: string): void
  (e: 'batch-delete'): void
  (e: 'add'): void
  (e: 'import'): void
  (e: 'refresh'): void
  (e: 'selection-change', rows: EnterpriseListVO[]): void
  ...
  <div class="table-wrapper">
    ...
    <div class="table-card">
      ...
      >
        ...
        <el-table-column prop="recruitmentStatus" label="招聘状态" width="110" align="center">
          <template #default="{ row }">
            <span :class="['tag-pill', row.recruitmentStatus === '招聘中' ? 'tag-green' : 'tag-gray']">
        ...
        <el-table-column prop="isDeleted" label="状态" width="80" align="center">
          <template #default="{ row }">
            <span :class="['status-pill', row.isDeleted ? 'status-off' : 'status-on']">
              {{ row.isDeleted ? '禁用' : '启用' }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="300" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <button
              type="button"
              :class="['op-btn', row.isDeleted ? 'green' : 'gray']"
              @click="emit('toggle-status', row)"
            >
              <svg v-if="row.isDeleted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
