...
const emit = defineEmits<{
  (e: 'detail', row: ChannelUnivListVO): void
  (e: 'edit', row: ChannelUnivListVO): void
  (e: 'toggle-status', row: ChannelUnivListVO): void
  (e: 'delete', id: string): void
  (e: 'batch-delete'): void
  (e: 'add'): void
  (e: 'refresh'): void
  (e: 'selection-change', rows: ChannelUnivListVO[]): void
      ...
      >
        ...
        <el-table-column prop="regionTag" label="地区标签" width="120" align="center">
          <template #default="{ row }">
            <span v-if="row.regionTag" :class="['tag-pill', row.regionTag === '香港' ? 'tag-orange' : 'tag-purple']">
              {{ row.regionTag }}
            </span>
