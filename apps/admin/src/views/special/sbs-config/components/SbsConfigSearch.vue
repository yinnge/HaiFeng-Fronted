...
const props = defineProps<{
  ...
  pilotYear: number | undefined
  testBeforeScore: boolean | undefined
}>()

const emit = defineEmits<{
  (e: 'update:universityName', val: string): void
  (e: 'update:isPilot', val: boolean | undefined): void
  (e: 'update:pilotYear', val: number | undefined): void
  (e: 'update:testBeforeScore', val: boolean | undefined): void
  (e: 'search'): void
  (e: 'reset'): void
}>()
...
const handleReset = () => {
  ...
  emit('update:isPilot', undefined)
  emit('update:pilotYear', undefined)
  emit('update:testBeforeScore', undefined)
  emit('reset')
}
</script>
      ...
      <el-form inline class="search-form">
        ...
        <el-form-item label="试点校">
          <el-select
            ...
            clearable
            style="width: 120px"
            @change="emit('update:isPilot', $event ?? undefined)"
          >
        ...
        <el-form-item label="试点年份">
          <el-input-number
            :model-value="pilotYear"
            :min="2000"
            :max="2099"
            controls-position="right"
            style="width: 130px"
            placeholder="年份"
            @change="emit('update:pilotYear', $event ?? undefined)"
          />
        </el-form-item>
        <el-form-item label="出分前校测">
          <el-select
            :model-value="testBeforeScore"
            placeholder="全部"
            clearable
            style="width: 120px"
            @change="emit('update:testBeforeScore', $event ?? undefined)"
          >
            <el-option label="是" :value="true" />
            <el-option label="否" :value="false" />
          </el-select>
