...
const handleSoftDelete = async (id: string, name: string) => {
  try {
    await ElMessageBox.confirm(`确定要禁用竞赛"${name}"吗？`, '提示')
...
const handleEnable = async (id: string, name: string) => {
  try {
    await ElMessageBox.confirm(`确定要启用竞赛"${name}"吗？`, '提示')
    const res = await enableCompetition(id)
    if (res.data.code === 200) {
      ...
      <el-table :data="tableData" v-loading="loading" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" width="300" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <button v-if="!row.isDeleted" class="action-pill action-warn" @click="handleSoftDelete(row.id, row.compName)">禁用</button>
            <button v-else class="action-pill action-success" @click="handleEnable(row.id, row.compName)">启用</button>
            <button class="action-pill action-danger" @click="handleHardDelete(row.id, row.compName)">删除</button>
          </template>
        </el-table-column>
      </el-table>
