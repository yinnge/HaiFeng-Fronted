...
const handleDelete = async (id: string) => {
  try {
    await ElMessageBox.confirm('确定要下架该指南吗？', '提示')
...
const handleBatchDelete = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要下架的指南')
    return
  }
  try {
    await ElMessageBox.confirm(`确定要下架选中的${selectedIds.value.length} 条指南吗？`, '提示')
    const res = await batchDeleteGuide(selectedIds.value)
    if (res.data.code === 200) {
