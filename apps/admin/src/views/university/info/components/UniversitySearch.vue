...
import type { UniversityQueryDTO } from '@/types/university/info'

const emit = defineEmits<{
  (e: 'search', params: Pick<UniversityQueryDTO, 'name' | 'provinceName' | 'category' | 'status'>): void
  (e: 'reset'): void
}>()
    ...
    <el-form :model="searchForm" inline class="search-form">
      <div class="filter-fields">
        <el-form-item label="院校名称">
          <el-input v-model="searchForm.name" placeholder="模糊搜索" clearable style="width: 180px" @keyup.enter="handleSearch" />
        </el-form-item>
...
.section-label {
  ...
  padding: 6px 16px; background: linear-gradient(135deg, #F97316, #FB923C);
  color: #fff; font-size: 13px; font-weight: 600; border-radius: 20px; margin-bottom: 20px;
}
...
.search-btn {
  ...
  cursor: pointer; transition: all 0.25s ease; box-shadow: 0 2px 8px rgba(249, 115, 22, 0.3);
}
...
.reset-btn {
  ...
  background: #fff; color: #6b7280; border: 1px solid #d1d5db; border-radius: 20px;
