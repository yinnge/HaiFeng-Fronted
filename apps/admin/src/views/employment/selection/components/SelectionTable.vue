...
import type { SelectionListVO } from '@/types/employment/selection'

const props = defineProps<{
  data: SelectionListVO[]
  loading: boolean
  total: number
  page: number
  size: number
...
const emit = defineEmits<{
  (e: 'detail', row: SelectionListVO): void
  (e: 'edit', row: SelectionListVO): void
  (e: 'delete', id: string): void
  (e: 'status-change', row: SelectionListVO, val: string): void
  (e: 'batch-delete'): void
  (e: 'add'): void
  (e: 'pre-validate'): void
  (e: 'import'): void
  (e: 'refresh'): void
  (e: 'selection-change', rows: SelectionListVO[]): void
...
<template>
  ...
  <div class="table-card">
    ...
    >
      ...
      <el-table-column prop="targetUnit" label="用人单位" min-width="180" show-overflow-tooltip />
      ...
      <el-table-column prop="regStartDate" label="报名开始" width="170" />

      <el-table-column prop="regEndDate" label="报名截止" width="170" />

      <el-table-column prop="positionStatus" label="职位状态" width="110" align="center">
        <template #default="{ row }">
          <span :class="['pill', positionStatusPill[row.positionStatus] || 'pill-gray']">{{ row.positionStatus }}</span>
        </template>
      </el-table-column>
