...
import type { TeacherDetailVO } from '@/types/employment/teacher'
...
watch(() => props.visible, async (val) => {
  if (val && props.currentId) {
    ...
    try {
      const res = await getTeacherDetail(props.currentId)
    ...
    <div v-loading="formLoading" class="dialog-content">
      <template v-if="detailData">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="教师资格证要求" :span="2">{{ detailData.teacherCertRequirement || '-' }}</el-descriptions-item>
          <el-descriptions-item label="教师资格证学科">{{ detailData.teacherCertSubject || '-' }}</el-descriptions-item>
          <el-descriptions-item label="普通话等级">{{ detailData.putonghuaLevel || '-' }}</el-descriptions-item>
          <el-descriptions-item label="其他证书要求" :span="2">{{ detailData.otherCertRequirement || '-' }}</el-descriptions-item>
          <el-descriptions-item label="工作经验">{{ detailData.workExperience || '-' }}</el-descriptions-item>
          <el-descriptions-item label="是否师范专业">{{ detailData.isNormalMajor || '-' }}</el-descriptions-item>
