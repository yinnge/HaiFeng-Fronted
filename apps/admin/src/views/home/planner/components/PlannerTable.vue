...
import type { PlannerListVO } from '@/types/home/planner'
...
const emit = defineEmits<{
  ...
  (e: 'toggle-status', row: PlannerListVO): void
      ...
      <el-table :data="data" stripe>
        ...
        <el-table-column prop="position" label="职位" width="140">
          <template #default="{ row }">
            <span v-if="row.position" class="position-text">{{ row.position }}</span>
            <span v-else class="dim-text">-</span>
          </template>
        </el-table-column>
        <el-table-column prop="region" label="地区" width="110">
          <template #default="{ row }">
            <span v-if="row.region" class="region-pill">{{ row.region }}</span>
            <span v-else class="dim-text">-</span>
          </template>
        </el-table-column>
        <el-table-column prop="specialty" label="专长" min-width="180" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.specialty" class="specialty-text">{{ row.specialty }}</span>
            <span v-else class="dim-text">-</span>
          </template>
        </el-table-column>
...
.position-text { font-weight: 500; color: #374151; }
.specialty-text { font-size: 13px; color: #4b5563; }
