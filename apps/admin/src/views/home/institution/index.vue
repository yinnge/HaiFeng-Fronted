...
<template>
  <div class="institution-page">
    ...
    <div class="page-header">
      <div class="page-title">培训机构管理</div>
      <div class="page-subtitle">维护培训机构信息、课程及展示状态</div>
    </div>
        ...
        <el-table :data="tableData" stripe>
          ...
          <el-table-column prop="phone" label="电话" width="140">
            <template #default="{ row }">
              <span class="dim-text">{{ row.phone || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="status" label="状态" width="100" align="center">
            <template #default="{ row }">
              <span class="status-tag" :class="row.status === 1 ? 'status-on' : 'status-off'">{{ statusLabel(row.status) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="260" align="center" fixed="right">
            <template #default="{ row }">
              <div class="action-group">
                ...
                <button type="button" class="action-btn action-delete" @click="handleDelete(row.id)">删除</button>
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- Dialog -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="700px" class="detail-dialog" :close-on-click-modal="false">
      <div v-loading="formLoading" class="detail-content">
        ...
        <template v-if="dialogMode === 'detail' && detailData">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="类别">
              <span class="type-tag">{{ detailData.type }}</span>
            ...
            <el-descriptions-item label="课程" v-if="detailData.courses && detailData.courses.length">
              <span v-for="(item, i) in detailData.courses" :key="i" class="course-tag">{{ item }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="排序值">{{ detailData.sortOrder }}</el-descriptions-item>
            <el-descriptions-item label="状态">
              <span class="status-tag" :class="detailData.status === 1 ? 'status-on' : 'status-off'">{{ statusLabel(detailData.status) }}</span>
            </el-descriptions-item>
        ...
        <template v-if="dialogMode !== 'detail'">
          <el-form :model="formData" label-width="110px" class="form-content">
            ...
            <el-form-item label="课程">
              <div class="dynamic-list">
                <div v-for="(item, index) in arrOrEmpty(formData.courses)" :key="index" class="dynamic-row">
                  <el-input v-model="formData.courses![index]" placeholder="请输入课程名称" style="width: 400px" />
                  <button type="button" class="row-remove-btn" @click="removeArrayItem(formData.courses, index)">删除</button>
                </div>
                <button type="button" class="row-add-btn" @click="addArrayItem(formData.courses)">+ 添加课程</button>
              </div>
            </el-form-item>
            <el-form-item label="图片 URL">
              <div class="dynamic-list">
                <div v-for="(item, index) in arrOrEmpty(formData.images)" :key="index" class="dynamic-row">
                  <el-input v-model="formData.images![index]" placeholder="请输入图片链接" style="width: 400px" />
                  <button type="button" class="row-remove-btn" @click="removeArrayItem(formData.images, index)">删除</button>
                </div>
                <button type="button" class="row-add-btn" @click="addArrayItem(formData.images)">+ 添加图片</button>
              </div>
            </el-form-item>
