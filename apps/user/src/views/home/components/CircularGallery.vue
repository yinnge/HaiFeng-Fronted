...
const props = withDefaults(defineProps<Props>(), {
  ...
  textColor: '#1f2937',
  borderRadius: 0.06,
  font: 'bold 26px "Microsoft YaHei", "PingFang SC", "Noto Sans SC", sans-serif',
  scrollSpeed: 2,
  scrollEase: 0.05,
})
...
// 空图 / 加载失败时的橙白色系占位图（避免 WebGL 纹理无内容）
const FALLBACK_IMAGE =
  'data:image/svg+xml;utf8,' +
  encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="700" height="900">' +
      '<rect width="700" height="900" fill="#fde8d4"/>' +
      '<circle cx="350" cy="400" r="72" fill="#f5a54a"/>' +
      '<rect x="250" y="512" width="200" height="16" rx="8" fill="#e8722a"/>' +
      '<rect x="290" y="544" width="120" height="10" rx="5" fill="#f5cbb3"/>' +
      '</svg>'
  )
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#1f2937', font = '26px sans-serif' }: TitleProps) {
    autoBind(this)
    this.gl = gl
    this.plane = plane
...
class Media {
  ...
  createShader() {
    ...
    img.src = this.image || FALLBACK_IMAGE
    img.onload = () => {
      texture.image = img
...
class App {
  ...
  createMedias(
    ...
    const duplicated = galleryItems.concat(galleryItems)
    this.medias = duplicated.map((data, index) => {
      return new Media({
  ...
  getActiveIndex(): number {
    if (!this.medias.length || !this.itemCount) return -1
    let bestIndex = -1
    ...
    return bestIndex < 0 ? -1 : bestIndex % this.itemCount
  }
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(x - this.start) > 8) this.dragged = true
...
onMounted(() => {
  ...
  const resolvedFont = props.font || 'bold 26px "Microsoft YaHei", "PingFang SC", sans-serif'
...
watch(
  ...
  (newProps) => {
    ...
    if (!containerRef.value) return
    const container = containerRef.value
    const resolvedFont = newProps.font || 'bold 26px "Microsoft YaHei", "PingFang SC", sans-serif'
