...
import { getSelectionList, getSelectionYears } from '@/api/employment/selection'
...
async function fetchYears() {
  try {
    const res = await getSelectionYears()
    yearOptions.value = res.data.data || []
  } catch {
    // 接口异常时回退为最近 6 年
    const currentYear = new Date().getFullYear()
    yearOptions.value = Array.from({ length: 6 }, (_, i) => String(currentYear - i))
  }
}
...
async function fetchList() {
  ...
  try {
    ...
    ElMessage.error(e?.message || '获取选调生岗位列表失败')
    jobs.value = []
    total.value = 0
  } finally {
    loading.value = false
...
onMounted(() => { fetchYears(); fetchList() })
</script>
