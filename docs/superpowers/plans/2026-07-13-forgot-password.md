# SMS 验证码 · 忘记密码功能 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为用户端增加忘记密码功能，支持手机号+图形验证码+短信验证码重置密码。

**Architecture:** 独立路由 `/forgot-password` 页面，两步表单向导（step1: 手机号+图形码→发短信 / step2: 短信码+新密码→重置）。API 封装在 `api/auth/forgot-password.ts`，类型定义在 `types/auth/forgot-password.ts`。
...
**Files:**
- Create: `apps/user/src/types/auth/forgot-password.ts`
...
**Files:**
- Create: `apps/user/src/api/auth/forgot-password.ts`
...
**Files:**
- Create: `apps/user/src/views/forgot-password/index.vue`
        ...
        <template v-if="step === 1">
          ...
          <div class="mb-6">
            ...
            <div class="flex gap-2">
              ...
              />
              <div
                class="flex h-[50px] w-[120px] cursor-pointer items-center justify-center overflow-hidden rounded-lg border border-gray-200 bg-gray-50"
                @click="fetchCaptcha"
              >
                <img
                ...
                <svg
                  v-if="form.captchaImage"
                  class="absolute ml-[88px] h-5 w-5 text-gray-400 hover:text-orange-500 cursor-pointer"
...
### Task 4: 更新登录页 — 添加「忘记密码？」链接

**Files:**
- Modify: `apps/user/src/views/login/index.vue:55`

- [ ] **在密码框下方、登录按钮上方添加忘记密码链接**

修改 `<div class="mb-6">` 密码框区块，在密码 input 下方添加右对齐的忘记密码链接：
...
同时登录页整体风格也要改成与首页一致的橙色主题（当前是蓝色）。将按钮改为橙色渐变，文字链接改为橙色：
...
并将页面背景改为 `bg-gradient-to-b from-slate-50 to-white`，以及将注册链接颜色改为 `text-orange-500 hover:text-orange-600`。
...
**Files:**
- Modify: `apps/user/src/router/index.ts:22`（在 Register 路由之后）
...
```bash
cd apps/user && npx vite --port 5175
```

- [ ] 访问 `/login` — 确认「忘记密码？」链接可见，登录页已适配橙色风格
- [ ] 访问 `/forgot-password` — 确认页面渲染正常，图形验证码加载
- [ ] 点击「获取短信验证码」— 步进到 step 2
- [ ] 倒计时功能正常
- [ ] 点击「重新获取」— 回到 step 1 重新输入图形码
- [ ] 点击「返回登录」— 跳转到 /login
